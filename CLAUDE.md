...
Based on **TXCraig1's yeast-prediction tables** (pizzamaking.com). Sources: `assets/bareme_fermentation_etendu_corrige.{xlsx,csv}` (35–80 °F, yeast 0.01–3 %) and `assets/bareme_fermentation_levure.{xlsx,csv}` (35–95 °F, yeast 0.01–1 %). `scripts/generate-fermentation-table.mjs` (`npm run fermentation-table`) **merges** them into `src/domain/FermentationTable.ts`: union of temps (35–95 °F) × the wide yeast axis (0.01–3 %), warm zone 81–95 °F null beyond 1 %. Full methodology + worked examples in `assets/fermentation-model.md`.
